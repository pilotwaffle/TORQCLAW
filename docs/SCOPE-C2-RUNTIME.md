...
| 3 | `gateway_profile_delegations` + `delegation_id` in origin capture | **SCHEMA DONE — WRITER STAGED, NOT LIVE** | `surfaceSecurity.ts` ledger + four guarded columns on `gateway_task_origins`. **`grantProfileDelegation` has zero production callers by decision**, so the three readers take their empty branch unconditionally and the LOCAL_EDGE exact-action fence (`server.ts` `carriesGrant`) has never executed in production. See the RESERVED-BY-DEFECT block on `surfaceSecurity.ts:grantProfileDelegation`; wiring it activates that fence for the first time and needs its own gate review. The prior "DONE" here referred to the schema and read as though the feature were live. |
