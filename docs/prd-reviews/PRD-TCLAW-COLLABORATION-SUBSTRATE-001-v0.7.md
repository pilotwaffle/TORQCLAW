...
This document is the sole normative v0.7 source. It replaces v0.1-v0.6 and their addenda. Earlier documents, commits `2f40e3a`, `f851aae`, and `cedae1f`, and review receipts remain evidence only.
...
Cursor rows are retained unchanged across membership removal and re-add; a re-added member resumes replay and unread computation from its retained acknowledged cursor.
...
`LIST_CHANNELS` orders by channel ID ascending, filters to active memberships, and includes archived rows only when requested. `ACK_CHANNEL_CURSOR` stores `max(existing,submitted)` but rejects a cursor beyond the greatest committed `channel_seq` in that channel. `SUBSCRIBE_CHANNEL`'s `highWaterCursor` is the greatest committed `channel_seq` visible to the caller in that channel at registration time.
...
- cursor rows are retained across membership removal, re-add, archive, and unarchive, and a re-added member resumes replay from its retained cursor;
...
| Prior source | Disposition in v0.7 |
|---|---|
