...
This document is the sole normative v0.10 source. It replaces v0.1-v0.9 and their addenda. Earlier documents, commits `2f40e3a`, `f851aae`, `cedae1f`, `1d981da`, `3e53e75`, and `8348ae2`, and review receipts remain evidence only.
...
Reference: Windows 11; 4 logical CPUs; 8 GiB RAM; SSD local NTFS database; Node.js LTS; Section 9 SQLite pragmas; 100,000 collaboration events; 30-minute harness; 60-second warm-up; at least 10,000 observations per phase; nearest-rank percentiles. Load phases: 10 concurrent clients for the baseline timeline, commit, and fan-out measurements; 25 concurrent readers continuously subscribing and reading plus one held slow consumer for the revocation writer-preference measurement.
...
- revocation-commit-to-last-write-boundary: p95 <= 150 ms, measured both with one deliberately slow consumer held at the 1 MiB queue limit (queue-depth independence) and under sustained reader arrival from 25 concurrent clients continuously subscribing and reading (writer-preference verification);
...
| Prior source | Disposition in v0.10 |
|---|---|
