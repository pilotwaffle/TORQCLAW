...
This document is the sole normative v0.4 source. It replaces v0.1-v0.3 and their addenda. Earlier documents and review receipts remain evidence only.
...
Subscriptions move absent -> backlog -> live -> closed. Authorization loss, slow consumer, unsubscribe, or socket close produces closed. Closed subscriptions never deliver.
...
Channel names are NFC-normalized, trimmed, 1-80 Unicode scalar values, and case-insensitively unique among active channels. Agent display names are NFC-normalized, trimmed, and 1-80 Unicode scalar values.

Message text is NFC-normalized and **1-16,384 UTF-8 bytes**. This is the only message-size rule.

Timeline pages contain at most **100 events and 512 KiB encoded**, whichever is reached first.
...
| Class | Commands |
|---|---|
| Idempotency-keyed | `CREATE_AGENT`, `SUSPEND_AGENT`, `RESTORE_AGENT`, `REVOKE_AGENT`, `ROTATE_PRINCIPAL_CREDENTIAL`, `REVOKE_PRINCIPAL_CREDENTIAL`, `CREATE_CHANNEL`, `ADD_CHANNEL_MEMBER`, `REMOVE_CHANNEL_MEMBER`, `ARCHIVE_CHANNEL`, `UNARCHIVE_CHANNEL`, `POST_CHANNEL_MESSAGE` |
...
`CREATE_CHANNEL` returns `CHANNEL_NAME_CONFLICT` when an active channel already has the normalized case-insensitive name. `UNARCHIVE_CHANNEL` returns the same code and leaves the target archived if another active channel has claimed its name; the operator must archive the conflicting channel first. `ACK_CHANNEL_CURSOR` returns `CURSOR_OUT_OF_RANGE` when the submitted cursor exceeds the greatest committed sequence visible in that channel.
...
6. idempotency lookup/conflict;
7. hidden-resource authorization;
...
The exhaustive session close reasons are `credential_revoked`, `principal_suspended`, `principal_revoked`, `operator_revoked`, `slow_consumer`, `unsubscribed`, `socket_closed`, and `recovery`.
...
CREATE UNIQUE INDEX collab_channels_active_name_ci
  ON collab_channels(lower(name)) WHERE state='active';
...
CREATE TABLE collab_session_bindings (
  ...
  close_reason TEXT CHECK(close_reason IS NULL OR close_reason IN (
    ...
    'operator_revoked','slow_consumer','unsubscribed',
...
Transaction checks enforce operator ownership, one active owner membership, agent ownership, event payload schema, cursor visibility, and legal state transitions.
...
- all timeline paths enforce 100 events/512 KiB;
...
Each slice is independently gated. Migration takes a mandatory pre-migration backup. After collaboration writes, rollback disables v2 and restores that backup; no downgrade migration exists.
...
| Prior source | Disposition in v0.4 |
|---|---|
