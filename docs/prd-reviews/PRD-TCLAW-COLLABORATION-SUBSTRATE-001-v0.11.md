...
This document is the sole normative v0.11 source. It replaces v0.1-v0.10 and their addenda. Earlier documents, commits `2f40e3a`, `f851aae`, `cedae1f`, `1d981da`, `3e53e75`, `8348ae2`, and `9fdc95b`, and review receipts remain evidence only.
...
`auth_epoch` is 1 at creation; every subsequent effective transition increments it by one. Suspend/revoke closes all sessions and subscriptions. Revoke also revokes every agent credential. Repeating a transition to the same state is idempotent; illegal transitions return `INVALID_PRINCIPAL_TRANSITION`.
...
Verification decrypts the kit with the supplied passphrase — proving the passphrase actually opens it; a checksum-only comparison is insufficient — verifies the recovered installation ID and the ciphertext checksum against the database, writes `recovery_kit_verified_at`, and emits a secret-free `recovery_kit_verified` audit row. Doctor warns when no verified kit record exists or the kit predates a pepper rotation. It cannot claim the offline file still exists.
...
The first response of a credential-producing command is `{principalId,credentialId,credential:"<secret>",credentialAvailable:true}`; every replay is `{principalId,credentialId,credentialAvailable:false}` with `credential` absent. `credential` is present if and only if `credentialAvailable` is true; the two shapes are pinned as byte fixtures in Section 10.
...
5. Cursor insert/update verifies an active membership and that the submitted sequence does not exceed the greatest committed `channel_seq` visible to that principal (the Section 7.4 caller-visible bound).
...
Conformance fixtures execute against a determinism harness: (a) an injected monotonic clock starting at the pinned instant 2026-01-01T00:00:00.000Z and advancing exactly 1 ms per server-generated timestamp, and (b) a deterministic UUID generator seeded per fixture with the fixture's identifier, producing UUID-shaped values in sequence. Every server-generated field in the Section 7.2 and 7.3 frame shapes — `sessionId`, `principalId`, `credentialId`, `eventId`, `subscriptionId`, `serverTime`, `occurredAt` — and the credential secret bytes in the two credential-shape fixtures are produced by these two mechanisms during fixture runs, so two independent implementations produce identical fixture bytes with no substitution table.
...
- a member removed at `channel_seq` N and re-added at M receives no event with `channel_seq` in the open interval (N, M) through timeline, backlog, or live fan-out, receives its own `member_added` event at M, and its retained cursor does not resurrect removal-window events; a first-time member's timeline from `afterCursor` 0 begins with its own `member_added` event; cursor rows survive archive and unarchive unchanged;
...
It also enforces cross-constraint feasibility: any declared encoded response or payload byte bound that exceeds the 64 KiB frame bound is a failure. It also verifies the caller-scoped owner predicate, the membership replay floor, the cursor lookup index, the timeline event object shape, and the normalization-ordering rule as required literals. It also verifies that the raw and encoded message bounds are equal, so the largest-legal-message pin is unambiguous, and that the Section 10 boundary fixtures state exact byte counts consistent with both bounds.
...
| Prior source | Disposition in v0.11 |
|---|---|
