...
**Status:** **PHASE 1 IMPLEMENTED + VERIFIED + SOL G2A APPROVED LOCALLY / UNMERGED + UNPUSHED + INACTIVE; PHASE 2A IMPLEMENTED + VERIFIED + SOL G2A APPROVED LOCALLY / UNMERGED + UNPUSHED + INERT + NOT SHIPPED; PHASE 2B+ NOT AUTHORIZED**
...
**Implementation authority:** the operator authorized the locally bounded Phase 1 implementation and the G1R-approved, inert Phase 2A implementation only. Phase 2A remains unshipped and non-live; Phase 2B+, V2 activation, cutover, live/authority changes, operational runs against real databases, merge, push, deployment, credentials, and certificate operations remain unauthorized.
...
**Phase 2A clarification:** **IMPLEMENTED + VERIFIED + SOL G2A APPROVED LOCALLY / UNMERGED + UNPUSHED / INERT AND NOT SHIPPED.** It is limited to offline/inert schema migration and non-authoritative reconciliation diagnostics; it issues no caller and creates no V2 session binding.
...
Each Phase 2A checksum is SHA-256 over UTF-8 migration-ID bytes, one `0x0A` byte, then the deterministic `AUTH_PHASE2A_PROGRAM_V1` serialization of the ordered manifest. Each framed step carries its kind, UTF-8 byte-length/name, and LF-pinned payload; assertion payloads identify exact versioned validators, DDL payloads are executed directly, and an empty receipt-boundary step binds receipt position without including receipt SQL or values. Source publishes a golden vector `{id, programUtf8Hex, sha256Hex}` for each migration and unit tests recompute the digest byte-for-byte. Canonical tuple hashing uses UTF-8 length-prefixed fields in order `(principal_id,surface_id,surface_kind,surface_role,connection_class,connection_class_revision)`; no delimiter encoding is allowed.
...
- Phase 2A diagnostic cases: exact length-prefixed hash golden vectors; active-principal/active-surface/class-matrix eligibility; `none` ineligible; `fixture_operator` unconditionally excluded from production diagnostics and covered only by isolated test helpers; zero eligible real tuple result; mismatch/invalid result is a `non_authoritative=1` record only and leaves V1 startup/authority/surface/credential/provisioning/revocation unchanged.
...
Overall authorization remediation status remains **designed/migration pending** because Phase 2B+ and later gates are pending. Phase 2A alone is locally implemented, independently verified, Sol G2A-approved, inert, unmerged, unpushed, and not shipped—never “authorization remediation shipped.”
