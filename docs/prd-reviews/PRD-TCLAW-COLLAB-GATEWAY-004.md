...
**Operator ruling 2026-08-11 (recorded at adoption; delegated decision; wording corrected same day per Sol review):** the at-rest retention trade-off flagged at adoption is **ACCEPTED — consciously, and scoped to the local operator trust boundary.** Historical model-proposed argument bytes remain at rest exactly as shipped, and that at-rest footprint includes `state.db`, its WAL, and any backups of either; read-time redaction protects the card/export path and does nothing for those at-rest copies. The acceptance rests on two facts, not on a coverage claim: erasure is irreversible while retention preserves the audit and rollback record, and the store is local to the single operator. What this ruling does NOT settle, verified against source at adoption: the shipped pre-C2 path still emits raw proposed args in `PENDING_APPROVAL` event metadata (`dispatch.ts:318`, `args: error.args` — the exact legacy behavior the prop-8 card path replaces), and `GET_SAFE_EXPORT` being redacted does not prove every console or storage path is. Therefore **"raw args never on wire/log" and the DB/WAL/backup protection posture remain mandatory built-artifact gates (§5(c)) before any C1/C2 runtime authorization** — this ruling accepts retention; it does not waive those gates. If at-rest erasure is ever wanted, it is a separately scoped operator lane, never a C1/C2 change.
