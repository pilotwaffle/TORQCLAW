...
| `PRD-TCLAW-AGENT-PARTICIPATION-007.md` (v0.1, stale) | Agents as channel participants | **S1–S3 SHIPPED** (identity, MCP tools, auto-reply + A3-c green, cron) | S4 (presence overlay — blocked on OQ-2 ruling: is "working now" an entitlement?), S5 (live push/departure), S6 (label honesty — overlaps C-S5-2). Owed: S5b (self-only `principalId` on CONNECTED, per G1R's disclosure analysis), §19 backpressure. |
